...
final userId = SupabaseClientManager.client.auth.currentUser?.id;
  ...
  // Create Items
  Future<void> addItem(String title,String description,String image,int price,int review,String bradName) async{
   final userID = SupabaseClientManager.client.auth.currentUser?.id;
  ...
  // update the itmes
  Future<void> updateItem(String title,String description,int price,String review,String bradName,DateTime update)async{
   await SupabaseClientManager.client.from('items').update(
    {
      'title': title,
      'description': description,
      'price': price,
